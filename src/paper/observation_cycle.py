    ...
    if observation_ledger.exists():
        ...
        if not obs.empty:
            ...
            if "outcome_status" in obs.columns:
                status["observations_pending"] = int(obs["outcome_status"].astype(str).eq("PENDING").sum())
            ...
            if "sent_to_broker" in obs.columns:
                status["sent_to_broker_any"] = bool(obs["sent_to_broker"].astype(bool).any())
        ...
        if not out.empty:
            ...
            if "sent_to_broker" in out.columns:
                status["sent_to_broker_any"] = bool(
                    status["sent_to_broker_any"] or out["sent_to_broker"].astype(bool).any()
                )
